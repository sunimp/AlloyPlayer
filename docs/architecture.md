...
AlloyPlayer 1.0.0 将播放能力拆成平台无关核心、具体播放引擎、UIKit 承载层、SwiftUI 桥接层和列表播放。默认播放入口不链接第三方缓存运行时，缓存等增强能力通过可选 product 接入。
...
flowchart LR
    ...
    Umbrella --> UIKit[AlloyPlayerUIKit]
    Umbrella --> SwiftUI[AlloyPlayerSwiftUI]
    ...
    App -. optional product .-> Cache[AlloyPlayerHTTPMediaCacheSupport]
    Cache -. depends on .-> Core
    Cache -. depends on .-> HTTPMediaCache[HTTPMediaCache]
```
...
| `AlloyPlayerUIKit` | 播放器视图、默认控制层、手势控制和全屏协调 |
| `AlloyPlayerSwiftUI` | SwiftUI 播放视图、控制器和默认 SwiftUI 控制层 |
...
| UIKit 播放视图 | `AlloyPlayerUIKit.AlloyPlayerView` |
...
| HTTPMediaCache | 可选 product `AlloyPlayerHTTPMediaCacheSupport` |

`RenderHostView`、`FloatingPlaybackView`、`VisibilityEvaluator`、`KVOManager` 和内部 logger 不是公开扩展点。业务侧应通过上表入口接入。

## 主路径

1. 普通调用方直接创建 `AlloyPlayerUIKit.AlloyPlayerView(source:)`，或使用 `AlloyPlayerController(source:)` + `AlloySwiftUIPlayerView`。
2. 高级调用方创建 `PlaybackSession`，通常通过 `AlloyPlayerFactory.makeDefaultSession()` 或自定义 `PlaybackEngine`。
...
- 缓存支持不进入 `AlloyPlayer` umbrella product 的依赖图，`AlloyPlayerHTTPMediaCacheSupport` 只负责生成代理播放源，不隐式修改播放器状态。
