//
//  AlloyHTTPMediaCacheSupportTests.swift
...
@Suite("AlloyHTTPMediaCacheSupport", .serialized)
struct AlloyHTTPMediaCacheSupportTests {
    ...
    func configureRequestHeadersForCacheDownloader() async {
        ...
        await AlloyHTTPMediaCacheSupport.configureRequestHeaders(headers)
    ...
    func clearRequestHeaders() async {
        await AlloyHTTPMediaCacheSupport.configureRequestHeaders(["Authorization": "Bearer token"])

        await AlloyHTTPMediaCacheSupport.configureRequestHeaders(nil)
    ...
    func defaultConfigurationClearsStaleRequestHeaders() async throws {
        await AlloyHTTPMediaCacheSupport.configureRequestHeaders(["Authorization": "Bearer stale"])
        let originalURL = try #require(URL(string: "https://example.com/plain-video.mp4"))

        _ = try await AlloyHTTPMediaCacheSupport.proxyURL(
            for: originalURL,
