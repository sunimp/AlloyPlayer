...
//
    ...
    struct AlloyUIKitPlayerRepresentable<Controls: View>: UIViewRepresentable {
        ...
        func makeUIView(context _: Context) -> AlloyPlayerUIKit.AlloyPlayerView {
            let playerView = AlloyPlayerUIKit.AlloyPlayerView(session: controller.session)
            playerView.backgroundColor = .black
            playerView.controlOverlay = SwiftUIHostingControlOverlay(controller: controller, controls: controls)
            return playerView
        }

        func updateUIView(_: AlloyPlayerUIKit.AlloyPlayerView, context _: Context) {}
    }
